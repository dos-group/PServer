...
public abstract class AbstractMatrix implements Matrix {
    ...
    @Override
    public Matrix applyOnElements(final DoubleFunction1Arg mf) {
        for (int i = 0; i < numRows(); ++i) {
            for (int j = 0; j < numCols(); ++j) {
                set(i, j, mf.apply(this.get(i, j)));
            }
        }
        return this;
    }
    ...
    @Override
    public Matrix applyOnElements(final Matrix m2, final DoubleFunction1Arg mf) {
        for (int i = 0; i < numRows(); ++i) {
            for (int j = 0; j < numCols(); ++j) {
                set(i, j, mf.apply(m2.get(i, j)));
            }
        }
        return this;
    }
    ...
    @Override
    public Matrix applyOnElements(final Matrix m2, final DoubleFunction2Arg mf) {
        Preconditions.checkState(m2.numRows() == this.numRows());
        Preconditions.checkState(m2.numCols() == this.numCols());
        for (int i = 0; i < numRows(); ++i) {
            for (int j = 0; j < numCols(); ++j) {
                set(i, j, mf.apply(this.get(i, j), m2.get(i, j)));
            }
        }
        return this;
    }
