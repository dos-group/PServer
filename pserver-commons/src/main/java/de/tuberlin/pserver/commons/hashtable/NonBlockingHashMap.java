 ...
 * @since 1.5
 * @author Cliff Click
 * @param <TypeK> the primitiveType of keys maintained by this map
 * @param <TypeV> the primitiveType of mapped values
 */
...
public class NonBlockingHashMap<TypeK, TypeV>
  ...
  // --- Minimum table partitionSize ----------------
  // Pick partitionSize 8 K/V pairs, which turns into (8*2+2)*4+12 = 84 bytes on a
  ...
  /** Create a new NonBlockingHashMap with default minimum partitionSize (currently set
   ...
   *  appropriate partitionSize.  Large numbers here when used with a small count of
   *  elements will sacrifice space for a small amount of time gained.  The
   *  initial partitionSize will be rounded up internally to the next larger power of 2. */
  ...
  private final void initialize( int initial_sz ) {
    ...
    // Double partitionSize for K,V pairs, add 1 for CHM and 1 for hashes
  ...
  /** Returns <tt>partitionSize() == 0</tt>.
   *  @return <tt>partitionSize() == 0</tt> */
  @Override 
  ...
  private static final class CHM<TypeK,TypeV> {
    ...
    private final Object[] resize( NonBlockingHashMap topmap, Object[] kvs) {
      ...
      // No copy in-progress, so start one.  First up: compute new table partitionSize.
      int oldlen = len(kvs);    // Old count of K,V pairs allowed
      int sz = size();          // Get current table count of active K,V pairs
      int newsz = sz;           // First partitionSize estimate

      // Heuristic to determine new partitionSize.  We expect plenty of dead-slots-with-keys
      ...
      if( sz >= (oldlen>>2) ) { // If we are >25% full of keys then...
        ...
        if( sz >= (oldlen>>1) ) // If we are >50% full of keys then...
          newsz = oldlen<<2;    // Double double partitionSize
      }
      ...
      if( newsz <= oldlen && // New table would shrink or hold steady?
        ...
        newsz = oldlen<<1;      // Double the existing partitionSize
      ...
      int log2;
      for( log2=MIN_SIZE_LOG; (1<<log2) < newsz; log2++ ) ; // Compute log2 of partitionSize
      ...
      // guess at 32-bit pointers; 64-bit pointers screws up the partitionSize calc by
      ...
      // Double partitionSize for K,V pairs, add 1 for CHM
