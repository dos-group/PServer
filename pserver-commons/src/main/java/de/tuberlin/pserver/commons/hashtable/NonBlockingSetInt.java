...
public class NonBlockingSetInt extends AbstractSet<Integer> implements Serializable {
   ...
   * version of contains, requires a primitiveType-check and auto-unboxing.  When
   ...
   * version of remove, requires a primitiveType-check and auto-unboxing.  When
   ...
   * the partitionSize is only ever approximate.  Updates due to the calling thread are
  ...
  private static final class NBSI {
    ...
    public boolean add( final int i ) {
      ...
      int j = i;                // The bit partitionIndex being added
    ...
    // Must grow the current array to hold an element of partitionSize i
    private NBSI install_larger_new_bits( final int i ) {
      if( _new == null ) {
        ...
        // CAS to install a new larger partitionSize.  Did it work?  Did it fail?  We
        // don't know and don't care.  Only One can be installed, so if
        // another thread installed a too-small partitionSize, we can't help it - we
        // must simply install our new larger partitionSize as a nested-resize table.
    ...
    private NBSI help_copy() {
      ...
      for( int i=0; i<HELP; i++ ) {
        int j = idx+i*64;
        j %= (top_nbsi._bits.length<<6); // Limit, wrap to array partitionSize; means we retry indices
